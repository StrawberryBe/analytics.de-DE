...
In dem oben aufgeführten Beispiel werden 553 Hits erzeugt, die im Cache-System verarbeitet werden sollen.
...
The number of Unique Visitors in a web-server log is calculated as the different distinct combinations of *`IP Address`* and *`User Agent`* in the Web log. Jede einzigartige Kombination dieser beiden Elemente wird als Unique Visitor berechnet. Wenn die Spalte [!UICONTROL Benutzeragent] leer ist (oder nicht im Webprotokoll eingeschlossen ist), können wir nicht die Anzahl an Unique Visitors ermitteln, sodass der gesamte Upload als ein Unique Visitor zählt (selbst wenn mehrere IP-Adressen vorliegen).
