...
| `click_action_type` | Wird nicht mehr verwendet. Linktyp des alten Clickmap-Werkzeugs.<br>0: HREF-URL<br>1: Benutzerspezifische ID<br>2: JavaScript onClick-Ereignis<br>3: Formularelement | tinyint unsigniert |
...
| `color` | Farbtiefen-ID basierend auf dem Wert der c_color-Spalte. Verweist auf die Nachschlagetabelle &quot;color_depth.tsv&quot;. | smallint unsigniert |
| `connection_type` | Numerische ID, die den Verbindungstyp darstellt. Die Variable, die in der Dimension „Verbindungstyp“ verwendet wurde. Verweist auf die Nachschlagetabelle connection_type.tsv. | tinyint unsigniert |
...
| `country` | Numerische ID, die das Land angibt, aus dem der Treffer stammt. Adobe arbeitet mit Digital Envoy zusammen, um die IP-Adresse dem Land zuzuordnen. Verwendet die Suche &quot;country.tsv&quot;. | smallint unsigniert |
...
| `curr_factor` | Bestimmt die Dezimalstelle für die Währung und wird zur Währungsumrechnung verwendet. USD verwendet beispielsweise zwei Dezimalstellen, sodass dieser Spaltenwert 2 ist. | tinyint |
...
| `cust_visid` | Wenn eine benutzerdefinierte Besucher-ID festgelegt ist, wird sie in dieser Spalte ausgefüllt. | varchar(255) |
...
| `duplicate_events` | Listen jedes Ereignisses, das als Duplikat gezählt wurde. | varchar(255) |
...
| `evar1 - evar250` | Benutzerdefinierte Variablen 1–250. Jede Organisation verwendet eVars anders. Der beste Ort für weitere Informationen darüber, wie Ihre Organisation die jeweiligen eVars füllt, wäre ein für Ihr Unternehmen spezifisches Lösungsdesign-Dokument. | varchar(255) |
| `event_list` | Kommagetrennte Liste numerischer IDs, die die durch den Treffer ausgelösten Ereignisse darstellen. Enthält sowohl die Standardereignisse als auch die benutzerdefinierten Ereignisse 1–1000. Verwendet Ereignis.tsv-Suche. | text |
...
| `first_hit_ref_type` | Numerische ID des Typs der verweisenden Stelle der allerersten verweisenden Stelle des Besuchers. Verwendet Werber_type.tsv-Suche. | tinyint unsigniert |
...
| `geo_country` | Abkürzung für das Land, aus dem der Treffer stammt, basierend auf der IP. Adobe arbeitet mit Digital Envoy zusammen, um die IP-Adresse dem Land zuzuordnen. | char(4) |
| `geo_dma` | Numerische ID des demografischen Bereichs, aus dem der Treffer stammt, basierend auf der IP. Adobe arbeitet mit Digital Envoy zusammen, um die IP-Adresse dem demografischen Bereich zuzuordnen. | int unsigniert |
| `geo_region` | Name des Bundeslands oder der Region, aus der der Treffer stammt, basierend auf der IP. Adobe arbeitet mit Digital Envoy zusammen, um die IP-Adresse dem Bundesland/der Region zuzuordnen. | char(32) |
| `geo_zip` | Die Postleitzahl, von der der Treffer stammt, basierend auf der IP. Adobe arbeitet mit Digital Envoy zusammen, um die IP-Adresse mit der Postleitzahl abzustimmen. | varchar(16) |
...
| `last_purchase_time_gmt` | Variable, die in der Dimension „Tage seit letztem Kauf“ verwendet wird. Zeitstempel (in Unix-Zeit) des letzten Kaufs. Bei Erstkäufen und Besuchern, die noch keinen Einkauf getätigt haben, ist dieser Wert 0. | int |
...
| `mobileappid` | ID der mobilen App. Stores the application name and version in the following format:[AppName] [BundleVersion] | varchar(255) |
...
| `mobilecampaignmedium` | Marketingmedium wie Banner oder E-Mail. Erfasst durch App-Akquise. | varchar(255) |
...
| `mobilecampaignsource` | Ursprünglicher Werber, z. B. Newsletter oder Social Media-Netzwerk. Erfasst durch App-Akquise. | varchar(255) |
| `mobilecampaignterm` | Bezahlte Suchbegriffe oder andere Begriffe, die Sie mit dieser Akquisition verfolgen möchten. Erfasst durch App-Akquise. | varchar(255) |
...
| `mobiledevice` | Mobilgerätname. Unter iOS als kommagetrennte 2-Ziffern-Zeichenfolge gespeichert. Die erste Zahl steht für die Gerätegenerierung, die zweite für die Gerätefamilie. | varchar(255) |
...
| `new_visit` | Flag, die bestimmt, ob der aktuelle Treffer ein neuer Besuch ist. Wird von Adobe-Servern nach einer 30-minütigen Besuchsinaktivität festgelegt. | tinyint unsigniert |
...
| `p_plugins` | Wird nicht mehr verwendet. Liste der für den Browser verfügbaren Plugins. Verwendung der JavaScript-Funktion navigator.plugins(). | text |
...
| `page_type` | Wird zum Füllen der Dimension „Seiten nicht gefunden“ verwendet; nur für 404-Seiten. Diese Variable sollte entweder leer sein oder &quot;ErrorPage&quot;enthalten. | char(20) |
...
| `post_ columns` | Enthält den in diesen Berichten ultimativ verwendeten Wert. Jede Post-Spalte wird nach serverseitigen Logik-, Verarbeitungsregeln- und VISTA-Regeln gefüllt. Adobe empfiehlt in den meisten Fällen die Verwendung von Post-Spalten. | Siehe entsprechende Nicht-Post-Spalte |
| `prev_page` | Nicht verwendet. Adobe-proprietäre Kennung der vorherigen Seite. | int unsigniert |
| `product_list` | Produktliste, so wie sie von der Variable der Produkte übergeben wurde. Produkte werden durch Kommas getrennt, während einzelne Produkteigenschaften durch Semikolons getrennt werden. | text |
| `product_merchandising` | Nicht verwendet. Verwenden Sie stattdessen product_Liste. | text |
| `prop1` – `prop75` | Benutzerspezifische Traffic-Variablen 1-75. | varchar(100) |
...
| `search_engine` | Numerische ID, die die Suchmaschine darstellt, die den Besucher auf Ihre Site verwiesen hat. Verwendet die Suche search_engine.tsv. | smallint unsigniert |
...
| `service` | Nicht verwendet. Verwenden Sie stattdessen page_Ereignis. | char(2) |
| `socialaccountandappids` | Wird nicht mehr verwendet. Social-Konto und App-IDs | varchar(255) |
| `socialassettrackingcode` | Wird nicht mehr verwendet. Variable &quot;Social Kampagne&quot; | varchar(255) |
| `socialauthor` | Wird nicht mehr verwendet. Social-Autorenvariable | varchar(255) |
| `socialcontentprovider` | Wird nicht mehr verwendet. Sozialen Plattformen/Eigenschaften | varchar(255) |
| `socialinteractiontype` | Wird nicht mehr verwendet. Social-Interaktionstyp | varchar(255) |
| `sociallanguage` | Wird nicht mehr verwendet. Social-Sprache | varchar(255) |
| `sociallatlong` | Wird nicht mehr verwendet. Social Breitengrad/Längengrad | varchar(255) |
| `socialowneddefinitioninsighttype` | Wird nicht mehr verwendet. Insight-Typ einer eigenen Social-Definition | varchar(255) |
| `socialowneddefinitioninsightvalue` | Wird nicht mehr verwendet. Insight-Wert für Social-eigene Definitionen | varchar(255) |
| `socialowneddefinitionmetric` | Wird nicht mehr verwendet. Soziale eigene Definitionsmetrik | varchar(255) |
| `socialowneddefinitionpropertyvspost` | Wird nicht mehr verwendet. Soziale eigene Definitionseigenschaft vs. Beitrag | varchar(255) |
...
| `socialownedpropertyname` | Wird nicht mehr verwendet. Eigene Social-Eigenschaftsname | varchar(255) |
| `socialownedpropertypropertyvsapp` | Wird nicht mehr verwendet. Soziale eigene Eigenschaft vs. App | varchar(255) |
| `state` | Without context I&#39;m afraid I can&#39;t assess which version is the correct one. | varchar(50) |
| `stats_server` | Keine Verwendung. Interner Adobe-Server, der den Treffer verarbeitet hat. | char(30) |
| `t_time_info` | Ortszeit für den Besucher. Format wie folgt: M/D/YYYY HH:MM:SS Monat (0-11, 0=Januar) Zeitzonenversatz (in Minuten) | varchar(100) |
...
| `user_hash` | Keine Verwendung. Hash der Report Suite-ID. Verwenden Sie stattdessen den Benutzernamen. | int unsigniert |
| `user_server` | Variable, die in der Dimension „Server“ verwendet wurde. | varchar(100) |
| `userid` | Keine Verwendung. Die numerische ID für die Report Suite-ID. Verwenden Sie stattdessen den Benutzernamen. | int unsigniert |
...
| `va_closer_id` | Numerische ID, anhand derer die Dimension „Letztkontaktdetails“ identifiziert wird. Die Suche nach dieser ID erfolgt im Marketing Kanal Manager. | tinyint unsigniert |
| `va_finder_detail` | Die Variable, die in der Dimension „Erstkontaktdetail“ verwendet wird. | varchar(255) |
| `va_finder_id` | Numerische ID, anhand derer die Dimension „Erstkontakt Kanal“ identifiziert wird. Die Suche nach dieser ID erfolgt im Marketing Kanal Manager. | tinyint unsigniert |
...
| `videoplayername` | Videoplayername | varchar(255) |
| `videoqoebitrateaverageevar` | Durchschnittliche Bitrate der Videoqualität | varchar(255) |
| `videoqoebitratechangecountevar` | Anzahl der Videoqualitätsänderungen | varchar(255) |
| `videoqoebuffercountevar` | Anzahl der Videopuffer | varchar(255) |
| `videoqoebuffertimeevar` | Videoqualitätspufferzeit | varchar(255) |
| `videoqoedroppedframecountevar` | Anzahl der Dropped Frames in Videoqualität | varchar(255) |
| `videoqoeerrorcountevar` | Anzahl der Videoqualitätsfehler | varchar(255) |
| `videoqoeextneralerrors` | Externe Fehler in Videoqualität | text |
| `videoqoeplayersdkerrors` | SDK-Fehler in Videoqualität | text |
| `videoqoetimetostartevar` | Videoqualitätszeit bis zum Beginn | varchar(255) |
...
| `visid_timestamp` | Wenn die Besucher-ID neu generiert wurde, wird der Zeitstempel (in Unix-Zeit) des Zeitpunkts bereitgestellt, zu dem die Besucher-ID generiert wurde. | int |
...
| `visit_ref_type` | Numerische ID des Typs der verweisenden Stelle der ersten verweisenden Stelle des Besuchs. Verwendet die Nachschlagetabelle Werber_type.tsv. | tinyint unsigniert |
| `visit_referrer` | Die erste verweisende Stelle des Besuchs. | varchar(255) |
| `visit_search_engine` | Numerische ID der ersten Suchmaschine des Besuchs. Verwendet die Suchtabelle search_engine.tsv. | smallint unsigniert |
