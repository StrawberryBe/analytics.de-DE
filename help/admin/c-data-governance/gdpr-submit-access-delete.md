...
Hierzu müssen Sie auch die im Rahmen der DSGVO-Zugriffsanfrage von Adobe Analytics zurückgegebenen Daten überprüfen, bevor Sie sie an das Datensubjekt senden. Besonders vorsichtig sollten Sie sein, wenn Sie Personen-IDs verwenden und nicht nur Daten zurückgeben, in denen diese ID enthalten ist, sondern auch Daten für andere Hits auf gemeinsam genutzten Geräten, auf denen die entsprechende ID manchmal genutzt wurde ([ID-Erweiterung](../../admin/c-data-governance/gdpr-analytics-ids.md#section_D55C0722BC834118BE6F958C30AD5913)).
...
Sie können DSGVO-Zugriffs- und -Löschanfragen über unser [DSGVO-UI-Portal](https://www.adobe.io/apis/cloudplatform/gdpr/docs/alldocs.html#!api-specification/markdown/narrative/gdpr/using-gdpr-ui.md) oder über unsere [GDPR-API](https://www.adobe.io/apis/cloudplatform/gdpr/docs/alldocs.html#!api-specification/markdown/narrative/gdpr/use-cases/gdpr-api-overview.md) einreichen.

>[!NOTE]
>
>Die GDPR-API unterstützt Batch-Übermittlungen für mehrere Benutzer in einer einzelnen Anforderung. Die Unterstützungsgrenze liegt momentan bei 1000 separaten Benutzern (pro Benutzer können mehrere IDs vorliegen) in einer einzelnen JSON-Anfragedatei.

## JSON-Beispielanfrage {#section_DB9DE6492FE740918F91D413E7BAB88F}

Hier sehen Sie den JSON-Abschnitt, der über die DSGVO-API oder -UI eingereicht werden kann und mit dem die DSGVO-Verarbeitung für drei Benutzer angefragt wird.
...
* Die Felder „type“ und „namespace“ werden im Abschnitt [Namespaces](../../admin/c-data-governance/gdpr-namespaces.md#concept_26C6392D92194BC1BA3986A144AF285D) detailliert beschrieben.
* Die Felder „description“ werden ignoriert.
* Die Felder „key“ können beliebige Werte enthalten. Wenn Sie über eine interne ID zum Verfolgen von DSGVO-Anfragen verfügen, können Sie hier den Wert ablegen, um die Zuordnung von Anfragen im Adobe-System zu denen in Ihren eigenen Systemen zu vereinfachen.
...
* Personendateien - Abgeleitet von Treffern, die eine übereinstimmende ID-Personenbeschreibung enthalten
...
* Gerätedateien - Abgeleitet von Treffern, bei denen eines der Felder mit einer angegebenen ID-DEVICE übereinstimmt, aber keine Übereinstimmung mit einer angegebenen ID-PERSON
...
Bei Löschanfragen werden keine Daten zurückgegeben. Stattdessen wird ein Status an die DSGVO-API übermittelt, der die erfolgreiche Bearbeitung der Anfrage angibt.

## DSGVO-Verarbeitung Ihrer Daten testen {#section_FBA843DBFAE64D979D8DB8A3C56784D7}
...
Mit einer normalen Konfiguration kann die Verarbeitung von DSGVO-Anfragen jedoch nicht erst an diesen Test-Report Suites getestet werden, bevor sie auf die Produktions-Report Suites angewendet wird. Grund hierfür ist, dass eine DSGVO-Anfrage automatisch auf alle Report Suites in der Experience Cloud-Organisation angewendet wird, was häufig allen Report Suites für Ihr Unternehmen entspricht.

Es gibt jedoch einige Möglichkeiten, wie Sie Ihre DSGVO-Verarbeitung vor der Anwendung auf all Ihre Report Suites testen können:

* Eine Option ist die Einrichtung einer separaten Experience Cloud-Organisation, die nur Test-Report Suites enthält. Verwenden Sie dann diese Experience Cloud-Organisation für Ihren DSGVO-Test und Ihre normale Experience Cloud-Organisation für die eigentliche DSGVO-Verarbeitung.
