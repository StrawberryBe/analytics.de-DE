...
seo-title: Best Practices für Beschriftungen
...
Bevor Sie herausfinden können, welche Beschriftungen auf welche Variablen/Felder angewendet werden sollen, müssen Sie zunächst die IDs verstehen, die Sie in Ihren Analytics-Daten erfassen, und entscheiden, welche Variablen für Datenschutzanforderungen verwendet werden. Der Datenschutz erweitert den Umfang dessen, was als ID betrachtet werden kann. IDs lassen sich allgemein in zwei Klassen unterteilen: direkt identifizierbare (Identitätsbeschriftung: I1) und indirekt identifizierbare IDs (Identitätsbeschriftung: I2).
...
* **Indirekt identifizierbare IDs (I2)**: ermöglichen nicht die direkte Identifikation einer Person, sondern müssen mit anderen Informationen (ob in Ihrem Besitz oder nicht) kombiniert werden, um die Person zu bestimmen. Hierzu zählen beispielsweise Kundentreue- oder im Unternehmens-CRM-System verwendete IDs, die für jeden Kunden eindeutig sind. Unter Datenschutz können die in den von Analytics verwendeten Tracking-Cookies gespeicherten anonymen IDs als indirekte Identifikation angesehen werden, auch wenn sie nur ein Gerät und nicht eine Einzelperson identifizieren können. auf einem freigegebenen Gerät können diese Cookies nicht zwischen verschiedenen Benutzern des Systems unterscheiden. Das Cookie kann beispielsweise nicht dazu verwendet werden, einen Computer zu finden, der das Cookie enthält, jedoch kann eine Person, die auf den Computer zugreifen kann und das Cookie dort findet, das Analytics-Cookie auf den Computer zurückführen.

   Auch eine IP-Adresse gilt als indirekt identifizierbar, da sie immer nur einem Gerät zugeordnet sein kann. Internetprovider können die IP-Adressen der meisten Benutzer jedoch ändern und tun dies regelmäßig. Deshalb können IP-Adressen mit der Zeit von mehreren Benutzern verwendet werden. Darüber hinaus kommt es nicht selten vor, dass mehrere Kunden eines Internetanbieters oder viele Mitarbeiter eines Unternehmens, die sich im selben Intranet befinden, eine externe IP-Adresse teilen. Because of this, Adobe will not support using an IP address as the ID for a [Data Privacy request.](/help/admin/c-data-governance/gdpr-submit-access-delete.md#submit-requests) Wenn eine zulässige ID im Rahmen einer Löschanfrage verwendet wird, löschen wir darüber hinaus die IP-Adressen, die mit dieser ID aufgetreten sind. Sie müssen entscheiden, ob es andere IDs gibt, die Sie erfassen, die unter diese Kategorie fallen können, I1 oder I2, aber nicht zur Verwendung als Unterscheidungs-ID für Datenschutzanforderungen geeignet sind.

Auch wenn Ihr Unternehmen viele verschiedene IDs in Ihren Analytics-Daten sammelt, können Sie nur eine Untergruppe dieser IDs für Datenschutzanforderungen verwenden. Hierfür gibt es folgende mögliche Gründe:

* In Ihren eigenen Systemen können Sie eine der IDs (z. B. die E-Mail-Adresse) einer anderen ID (z. B. der CRM-ID) zuordnen. Aus Gründen der Konsistenz entscheiden Sie sich dann dafür, die CRM-ID nur für Datenschutzanforderungen in Ihrer Datendatenverarbeitung zu verwenden.
...
Verwenden Sie diese Tabelle, um die IDs zu ermitteln, die Sie beim Senden von Datendatenschutzanforderungen an Analytics verwenden werden. Sobald Sie diese Informationen kennen, können Sie die anderen Beschriftungen, die Sie für Ihre Variablen verwenden sollten, leichter bestimmen.
  ...
  <tr> 
   <td colname="col1"> <p>Cookie-IDs </p> 
    <ul id="ul_CB43CEA3054E490585CBF3AB46F95B5B"> 
     ...
     <li id="li_7B6A9A788BBD47428315B3893FC07BC3"> <a href="https://marketing.adobe.com/resources/help/en_US/mcvid/" format="https" scope="external">Identity Service Cookie</a> (ECID), zuvor als Experience Cloud ID (MCID) bezeichnet </li> 
    </ul> </td> 
   <td colname="col2"> <p>Diese Cookies identifizieren ein Gerät oder genauer gesagt einen Browser für einen Benutzer eines Geräts. Auf einem gemeinsam genutzten Gerät, auf dem ein allgemeiner Login verwendet wird, kann diese ID für mehrere/alle Benutzer des Geräts gelten. Adobe has created some <a href="https://www.adobe.io/apis/cloudplatform/gdpr/services/allservices.htm" format="html" scope="external"> unified JavaScript </a> that you can place on your website to collect these cookies if you want to allow them to be used for Data Privacy requests. </p> <p>Benutzer des mobilen Adobe Analytics-SDK verfügen auch über eine Experience Cloud ID (ECID). Es gibt API-Aufrufe innerhalb des SDK, um diese ID zu lesen, sodass Sie Ihre App für eine Datenschutzanforderung erweitern können. </p> <p>Viele Unternehmen behandeln Browsercookie-IDs wie IDs gemeinsam genutzter Geräte. Infolgedessen können sie sich in Absprache mit ihren Juristen dafür entscheiden, die Verwendung dieser IDs als akzeptable IDs für Datenschutzanforderungen nicht zu unterstützen, oder sie können festlegen, dass bei Verwendung dieser IDs nur eine sehr begrenzte Menge an Daten zurückgegeben wird, oder sie akzeptieren sie nur für Löschanforderungen. </p> <p>Diese Cookies verfügen über eine ID-DEVICE-Beschriftung, die nicht geändert werden kann (sowie die Beschriftungen „I2“ und „DEL-DEVICE“). Die standardmäßige Adobe Analytics-Konfiguration gibt nur allgemeine Informationen zum Gerät, wie z. B. Gerätetyp, Betriebssystem, Browser usw., sowie Uhrzeit und Datum des Besuchs unter Verwendung dieser IDs zurück. Wenn Sie sich jedoch dafür entscheiden, diese IDs für Datenschutzanforderungen zu unterstützen (siehe unten), können Sie ACC-ALL-Beschriftungen hinzufügen oder entfernen, um den genauen Satz von Feldern zu konfigurieren, die für eine Datendatenschutzanforderung zurückgegeben werden sollen. </p> <p>Insbesondere wenn die Report Suite zu einer Mobile App gehört und Ihre Mobile App eine Anmeldung erfordert, gehen Sie möglicherweise davon aus, dass die Experience Cloud ID für das Gerät mit einem bestimmten Benutzer verknüpft ist, und versehen deshalb mehr Felder mit der ACC-ALL-Beschriftung, einschließlich der Namen der besuchten Seiten, aufgerufener Produkte usw. </p> <p>Hinweis:  Wenn Sie in Ihrer Datenschutzanfrage die Option "developIds"angeben, enthalten Ihre Anforderungen neben allen anderen angegebenen IDs immer Cookie-IDs. Weitere Informationen finden Sie unter <a href="/help/admin/c-data-governance/gdpr-id-expansion.md" format="dita" scope="local">ID-Erweiterung</a>. In diesen Instanzen geben Treffer, die außer einer Cookie-ID keine weiteren IDs aufweisen, im Rahmen der Zugriffsanfragen nur mit ACC-ALL beschriftete Daten zurück. </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col2"> <p>Manche Kunden speichern IDs in <a href="https://marketing.adobe.com/resources/help/en_US/sc/implement/props_eVars.html" format="html" scope="external">benutzerspezifischen Traffic-Variablen (Props) oder benutzerspezifischen Konversionsvariablen (eVars) </a>. Zwar wird am häufigsten die CRM-ID verwendet, jedoch sind auch andere IDs möglich, darunter E-Mail-Adressen, Anmeldenamen, Treuenummern oder Hashes dieser Werte. </p> 
    <ul id="ul_0B9492CF786046BB97E31CCF83A85FEA"> 
     <li id="li_D35B61CC6A8B485A8E09358A46D3F598">Wenn Sie eine dieser IDs für Datenschutzanforderungen verwenden möchten, sollten Sie dem Feld, das diese ID enthält, eine ID-PERSON-Bezeichnung geben. </li> 
    ...
    </ul> <p> Es ist selten, dass eine Report Suite mehr als eine oder zwei benutzerdefinierte Variablen mit IDs enthält, die Sie zur Identifizierung von Datensubjekten für Datenschutzanforderungen verwenden möchten. Möglicherweise sind den I1- oder I2-Beschriftungen mehrere Variablen zugeordnet, jedoch verfügt in der Regel nur eine von ihnen über ID-PERSON- oder ID-DEVICE-Beschriftungen. </p> </td> 
  </tr> 
  <tr> 
...
>[!NOTE]
>
>Bei Props spielt die Groß-/Kleinschreibung keine Rolle. eVars sind standardmäßig nicht von der Schreibweise abhängig, können jedoch über die Adobe-Kundenunterstützung entsprechend konfiguriert werden. Wenn Sie eine eVar mit Unterscheidung zwischen Groß- und Kleinschreibung und einer ID verwenden, ist es Ihre Verantwortung, bei der Übermittlung einer Datenschutzanforderung die richtige Groß-/Kleinschreibung zu verwenden, damit die in der Anforderung verwendete Groß-/Kleinschreibung mit der bei Treffern mit diesen IDs verwendeten Groß-/Kleinschreibung übereinstimmt.

Die Löschbeschriftungen DEL-DEVICE und DEL-PERSON sollten sparsam eingesetzt werden. Bei Anwendung auf eine Variable, die keine ID enthält, die im Rahmen der Datenschutzanforderung verwendet wurde, ändern sich die Zählungen (Metriken) in historischen Analytics-Berichten fast immer.
...
* The DEL-labels will result in these variables being [anonymized](/help/admin/c-data-governance/gdpr-labels.md#data-governance-labels) (the ID will be replaced with a random string prefixed with "Data Privacy-"). Dieser anonymisierte Wert wird in allen Instanzen des ursprünglichen Wertes ersetzt – in allen Hits, die über eine in der Anfrage enthaltene ID identifiziert werden. Wenn es sich beim ursprünglichen Wert in diesem Feld um eine dieser IDs handelte, ändern sich die Berichtsmetriken nicht.
...
* Beachten Sie, dass bei der Angabe einer DEL-DEVICE- oder DEL-PERSON-Beschriftung für eine beliebige Variable, die nicht zusätzlich als ID für die jeweilige Anfrage verwendet wird (einschließlich einer erweiterten ID), eindeutige Werte in der betreffenden Variable nur für Treffer anonymisiert werden, in denen eine bestimmte (oder erweiterte) ID vorkommt. Wenn andere Treffer denselben Wert enthalten, erfolgt unter den anderen Speicherorten keine Aktualisierung. Dadurch kann sich die Anzahl (Metriken) ändern.
   ...
   Wenn beispielsweise drei Treffer den Wert "foo"in eVar7 enthalten, aber nur einer von ihnen auch eine ID in einer anderen Variablen enthält, die für einen Löschvorgang gefunden wird, wird "foo"bei diesem Treffer in einen Wert wie "Data Privacy-123456789"geändert, während er in den anderen beiden Treffern unverändert bleibt. In einem Bericht, der Aufschluss über die Anzahl eindeutiger Werte für eVar7 gibt, werden nun mehr eindeutige Werte angezeigt als zuvor. Ein Bericht, der die obersten Werte für eVars anzeigt, kann "foo"mit nur zwei Instanzen (nicht 3 zuvor) enthalten, und der neue Wert wird auch mit einer einzigen Instanz angezeigt.

## Best Practices für Zugriffsbeschriftungen {#section_AC7E216F81C141FCA6A62F8836E06EE7}

Zwar verfügen nur wenige Felder über einige der anderen Beschriftungen, jedoch wird es häufig vorkommen, dass viele Felder ACC-Beschriftungen aufweisen. Die entsprechenden Zugriffsbeschriftungen hängen von den IDs ab, die Sie für Datenschutzanforderungen verwenden.
