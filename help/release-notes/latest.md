...
| ----------- | ---------- | ---------- |
| **SFTP-Upgrade** | 9. September 2022 | Zuvor hatten wir mitgeteilt, dass Adobe im September 2022 seine SFTP-Services (Secure File Transfer Protocol) aktualisieren würde, um die Sicherheit für die Dateiübertragung zu verbessern. Wir haben dieses Upgrade auf **Mitte bis Ende September 2022**. Nach dieser Änderung werden bestimmte SFTP-Client-Konfigurationen nicht mehr unterstützt. Dies wirkt sich nur auf Daten aus, die per SFTP an Adobe Analytics gesendet oder von Adobe Analytics abgerufen werden. Das FTP-Protokoll ist nicht betroffen. Um Service-Unterbrechungen zu vermeiden, stellen Sie bitte sicher, dass Ihre SFTP-Clients (Code, Tools, Services) die [hier](https://experienceleague.adobe.com/docs/analytics/export/ftp-and-sftp/secure-file-transfer-protocol/sftp-upgrade.html?lang=de) beschriebenen Anforderungen erfüllen. |
| **EOL für Data Workbench** | 9. September 2022 | Adobe beabsichtigt, Data Workbench zum **31. Dezember 2023** auslaufen zu lassen. Weitere Einzelheiten werden in Kürze mitgeteilt. |
| **Aktualisierung der Gerätesuche aufgrund von Google-Client-Hinweisen** | 9. September 2022 | Starten am **29. September 2022** verwendet Adobe zusätzlich zum Benutzeragenten Clienthinweise, wenn bestimmte Geräteinformationen für Treffer von Chromium-Browsern abgeleitet werden, z. B. Google Chrome und Microsoft Edge. Dies ist eine Reaktion auf den Plan von Google, die Informationen aus der Benutzeragenten-Zeichenfolge schrittweise zu reduzieren und stattdessen Daten über Client-Hinweise zu übermitteln. Weitere Informationen über Client-Hinweise finden Sie [hier](https://web.dev/user-agent-client-hints/).<p> Ab Oktober werden sowohl die AppMeasurement- als auch die Web-SDK-Sammelbibliotheken die Sammlung von Client-Hinweisen unterstützen und so konfiguriert sein, dass Client-Hinweise mit hoher Entropie gesammelt werden. Im Rahmen dieser Änderung verwendet Adobe künftig Device Atlas für alle Geräte-Suchvorgänge im Zusammenhang mit dem Benutzeragenten. Derzeit wird Device Atlas nur für mobile Zugriffe verwendet. Diese Aktualisierungen können zu geringfügigen Änderungen der Geräteinformationen führen, die bisher vom Benutzeragenten abgeleitet wurden, insbesondere über Browser, Browser-Typ, Betriebssystem, Betriebssystemtyp und Mobilgerät. |
