...
* Diese Änderungen an der Analytics-Verarbeitungs- und Berichterstellungs-Engine werden in der letzten Oktober-Woche bereitgestellt: Wir werden ein Problem beheben, bei dem die Beschriftungen für die Seiten- oder Link-Dimensionen fälschlicherweise als `Unknown`. Vor der Korrektur muss die Variable `Unknown` -Beschriftungen wurden möglicherweise falsch angezeigt, wenn ein Seitenname oder Linkname bei einem Treffer nicht übergeben wurde. Die Standardeinstellung ist [!UICONTROL Seiten-URL] und [!UICONTROL Link-URL], bzw. Diese Dimensionen wurden so konfiguriert, dass nicht zwischen Groß- und Kleinschreibung unterschieden wird. Mit dieser Fehlerbehebung werden die zukünftigen Berichte korrekt sein. Bei Berichten zu historischen Daten werden einige Berichtsergebnisse jedoch möglicherweise noch fälschlicherweise als `Unknown`. (AN-328030)
