...
>[!NOTE]
>
>Die vom Benutzer eingegebene "Auslieferungszeit" /" Tageszeit" gibt den Zeitpunkt an, zu dem der Bericht mit der Verarbeitung beginnen soll, nicht den Zeitpunkt, zu dem er tatsächlich bereitgestellt wird. Die tatsächliche Zeit, an der der Bericht bereitgestellt wird, basiert hauptsächlich darauf, wie lange es dauert (komplexe und große Berichte verarbeiten die Verarbeitung länger als einfache Berichte). Wenn beispielsweise ein Bericht 15 Minuten zu verarbeiten braucht, liegt die tatsächliche Bereitstellungszeit mindestens 15 Minuten nach der ursprünglich angegebenen "Lieferzeit" /" Tageszeit" .
>Darüber hinaus gibt es eine Reihe weiterer Faktoren, die die Verzögerung weiter erhöhen können, bevor der Bericht tatsächlich bereitgestellt wird:
>
> * **Zahlreiche verschiedene Zeitpläne desselben Typs gleichzeitig ausführen** (z. B. viele Dashboards usw.) kann das System überladen. Das Planungssystem erlaubt nur einige (bis 10) Berichte von beliebigen Typen, die gleichzeitig ausgeführt werden sollen. Wenn also mehr als 5-10 gleichzeitig geplant werden, müssen einige einige Berichte warten, bis alle Berichte abgeschlossen sind, bevor sie mit der Verarbeitung beginnen können. Dieses Problem kann reduziert werden, indem Sie die Berichte eines Unternehmens zu stagnierenden Zeiten außerhalb des Tages oder der Stunde planen und nicht gleichzeitig.
> * Abgesehen vom spezifischen Berichtstyp (Dashboards usw.) werden Berichte ebenfalls in Zeile gewartet, wenn das Unternehmen über **mehr als 15-20 jedes Berichtstyps verfügt (für alle anderen Berichtstypen)**. Dies kann durch die plötzliche Zeitplanung reduziert werden, anstatt dass viele Ausführungsvorgänge gleichzeitig ausgeführt werden.
> * **Probleme in Downstream Services** , auf die der Scheduler angewiesen ist, können ebenfalls die Auslieferung von Berichten beeinflussen. Wenn Sie beispielsweise die apis unabhängig verwenden, um Berichte auszuführen und die API-Anforderungswarteschlange auszufüllen, können Ihre geplanten Berichte langsam liefern, während Sie auf diese Ressource konkurrieren.
...
| Option | Beschreibung |
|--- |--- |
| Berichtsdateiname | Gibt den Namen des Berichts an. Das Standardformat `<report name> for <suite> - <report date range>`ist. Um einen benutzerspezifischen Namen festzulegen wählen Sie [!UICONTROL Benutzerdefiniert] aus. |
