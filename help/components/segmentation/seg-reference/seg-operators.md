...
| enthält | Gibt Elemente zurück, die mit den Unterzeichenfolgen der eingegebenen Werte vergleichbar sind. Wenn die Regel für „Seite“ z. B. „Suche“ enthält, stimmt dies mit allen Seiten überein, die die Unterzeichenfolge „Suche“ enthalten, z. B. „Suchergebnisse“, „Suche“ und „Suchen“. Bei der &quot;contains&quot;-Klausel wird in Adobe Analytics nicht zwischen Groß- und Kleinschreibung unterschieden, in Customer Journey Analytics wird jedoch zwischen Groß- und Kleinschreibung unterschieden. |
...
| stimmt überein mit | Gibt Elemente mit einer exakten Entsprechung für gegebene numerische oder Zeichenfolgenwerte wieder. Bei der &quot;matches&quot;-Klausel wird in Adobe Analytics und in Customer Journey Analytics zwischen Groß- und Kleinschreibung unterschieden. **Hinweis**: Verwenden Sie diesen Operator bei der Verwendung von Platzhalterfunktionen (Globbing). Beispiele für &quot;Globbing&quot;:<ul><li>`a*e` würde übereinstimmen  `ae`,  `abcde`,  `adobe`und  `a whole sentence`</li><li>`adob*` würde übereinstimmen  `adobe`,  `adobe analytics`und  `adobo recipe`</li><li>`*dobe` würde übereinstimmen  `dobe`,  `adobe`und  `cute little dobe`</li></ul> |
